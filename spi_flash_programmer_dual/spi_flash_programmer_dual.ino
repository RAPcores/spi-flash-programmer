...
#define POSITION_INTERRUPT_PIN 24
...
void setup_driver(TMC5160Stepper& driver) {
    ...
    //driver.pwm_freq(0);
    #define STEP_PULSE_DELAY_US 200 //80
}
...
void setup()
{
  ...
  pinMode(POSITION_INTERRUPT_PIN, INPUT_PULLUP);
...
void loop3() {
  ...
  if( SerialUSB.available() )
  {
    ...
    switch(cmd) {
      case '?':
        SerialUSB.println("hello");
        SerialUSB.print("POS_INT: "); SerialUSB.println( digitalRead(POSITION_INTERRUPT_PIN) );
        break;
      case 'S':
      ...
      case 's':
        ...
        for( int i = 0; i < 3200; i++)
        {
          if(0) for(int counter = 3; counter && ( ! digitalRead(POSITION_INTERRUPT_PIN) ); counter-- ) {
            delays_count++;
            delayMicroseconds(STEP_PULSE_DELAY_US);
          }

          while( ! digitalRead(POSITION_INTERRUPT_PIN) ); {
            delays_count++;
            delayMicroseconds(STEP_PULSE_DELAY_US);
          }
          ...
          //if( ! digitalRead(POSITION_INTERRUPT_PIN) ) delayMicroseconds(STEP_PULSE_DELAY_US);
          digitalWrite(STEP1,HIGH);
          delayMicroseconds(2);
          digitalWrite(STEP1,LOW);
          delayMicroseconds(2);
        ...
        SerialUSB.print("\n delays_count: "); SerialUSB.println(delays_count);
      case 'q':
        rapchip_cmd(1,0);     // 1 - Read Step Position
        rapchip_cmd(2,0);     // 2 - Read Quadrature
        rapchip_cmd(3,0);     // 3 - Read Quadrature
        break;
      ...
      case 'd':
        dir_x = ! dir_x;
        digitalWrite(DIR1,dir_x);
        SerialUSB.print("dir_x: ");
        SerialUSB.println(dir_x);
        break;

      case 'x':
        SerialUSB.print("  POS_X: ");
        SerialUSB.println(pos_x);
        break;
      ...
      case 'p':
        if(isDigit(SerialUSB.peek())){
          uint8_t pwm = SerialUSB.parseInt();
      ...
      // New Convention: uppercase statements mean it takes an argument.
      
      // P<new-value>
      // This updates the persistent variable which is then sent along with the command frame over SPI (cmd+packet).
      case 'P':
        if(isDigit(SerialUSB.peek())){
          tx_packet = SerialUSB.parseInt();
          SerialUSB.print("tx_packet: "); SerialUSB.println(tx_packet);
        }
        break;

      // Command to send. Also sends a packet over SPI.
      case 'C':
        if(isDigit(SerialUSB.peek())){
          uint8_t cmd = SerialUSB.parseInt();
          SerialUSB.print("cmd: "); SerialUSB.print(cmd);
          SerialUSB.print("  tx_packet: "); SerialUSB.println(tx_packet);
          rapchip_cmd(cmd,tx_packet);
        }
        break;

      case 'e':
        {
          static bool drive_enable = 0;
          drive_enable = ! drive_enable;
          digitalWrite(DRV_EN, drive_enable);
        }
        break;
...
void rapchip_cmd(uint8_t cmd, uint32_t tx_packet)
{
      ...
      SerialUSB.print("  RX: ");
      uint8_t resp1 = uspi_transfer(cmd);
      SerialUSB.print(resp1);
  ...
  */
      ...
      //Working
      SerialUSB.print("  RX32S: ");   
      int32_t respi = (((uint32_t)resp1 << 24) | ((uint32_t)resp2 << 16)  | ((uint32_t)resp3 << 8)  | ((uint32_t)resp4)); 
      SerialUSB.println(respi);
